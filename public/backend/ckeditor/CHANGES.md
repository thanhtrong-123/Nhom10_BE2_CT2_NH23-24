��システム情報の一部のサンプルも送信しますログイン時には、このデータを一時的に Google アカウントにリンクして、すべての Google アプリを安全に利用できるように保護します危険なイベントの発生を検出して警告します安全確認実行中…安全確認を数分前に実行しました{NUM_MINS,plural, =1{安全確認を 1 分前に実行しました}other{安全確認を {NUM_MINS} 分前に実行しました}}{NUM_HOURS,plural, =1{安全確認を 1 時間前に実行しました}other{安全確認を {NUM_HOURS} 時間前に実行しました}}安全確認を昨日実行しました{NUM_DAYS,plural, =1{安全確認を 1 日前に実行しました}other{安全確認を {NUM_DAYS} 日前に実行しました}}安全確認を実行しています。安全確認が完了しました。今すぐ確認安全確認を今すぐ実行安全確認を再実行実行中成功情報警告内容を確認$1 にもう一度権限を許可しますもう一度許可削除された権限を確認{NUM_SITES,plural, =1{しばらくアクセスしていない <b>1 件のサイト</b>から権限を削除しました}other{しばらくアクセスしていない <b>{NUM_SITES} 件のサイト</b>から権限を削除しました}}$1を削除しました$1、$2を削除しました$1、$2、$3を削除しました$1、$2、他 $3 個を削除しました{NUM_SITES,plural, =1{1 件のサイトの権限を削除しました}other{{NUM_SITES} 件のサイトの権限を削除しました}}{NUM_SITES,plural, =1{データ保護のため、2 か月間アクセスしていないサイトから権限を削除しました}other{データ保護のため、2 か月間アクセスしていないサイトから権限を削除しました}}{NUM_SITES,plural, =1{1 件のサイトの確認が完了しました}other{{NUM_SITES} 件のサイトの確認が完了しました}}$1 にもう一度権限を許可しました更新更新は<a target="_blank" href="$1">管理者</a>によって管理されていますChromium では、パスワード確認をご利用いただけません標準保護機能がオンになっています標準保護機能がオンになっています。セキュリティをさらに強化するには、保護強化機能を使用してください。保護強化機能がオンになっています<a target="_blank" href="$1">管理者</a>がセーフ ブラウジングをオフにしました拡張機能により、セーフ ブラウジングがオフになりましたセーフ ブラウジングを管理有害な可能性がある拡張機能から保護されています{NUM_EXTENSIONS,plural, =1{1 個の有害な可能性がある拡張機能がオフになっています。削除することもできます。}other{{NUM_EXTENSIONS} 個の有害な可能性がある拡張機能がオフになっています。削除することもできます。}}{NUM_EXTENSIONS,plural, =1{1 個の有害な可能性がある拡張機能を再度有効にしました}other{{NUM_EXTENSIONS} 個の有害な可能性がある拡張機能を再度有効にしました}}{NUM_EXTENSIONS,plural, =1{管理者が 1 個の有害な可能性がある拡張機能を再度有効にしました}other{管理者が {NUM_EXTENSIONS} 個の有害な可能性がある拡張機能を再度有効にしました}}拡張機能を確認{NUM_EXTENSIONS,plural, =1{Chrome ウェブストアから削除された <b>1 件の拡張機能</b>を確認}other{Chrome ウェブストアから削除された <b>{NUM_EXTENSIONS} 件の拡張機能</b>を確認}}{NUM_SITES,plural, =1{通知を多数受け取った 1 件のサイトを確認する}other{通知を多数受け取った {NUM_SITES} 件のサイトを確認する}}通知権限を確認{NUM_SITES,plural, =1{このサイトは最近多くの通知を送信しています。今後の通知を停止できます。}other{これらのサイトは最近多くの通知を送信しています。今後の通知を停止できます。}}$1 で通知を許可されています$1 では通知が許可されていません$1 では通知が許可されていません。次回このサイトにアクセスすると、確認が再度表示されます。{NUM_NOTIFICATION,plural, =1{1 日あたり約 1 件の通知}other{1 日あたり約 {NUM_NOTIFICATION} 件の通知}}許可しない$1 からの通知を許可しないこのサイトからの通知を常に許可する$1 からの通知を常に許可する許可しないが後で確認する$1 からの通知を許可しないが後で確認する{NUM_SITES,plural, =1{最近、通知を多数送信した <b>1 件のサイト</b>を確認する}other{最近、通知を多数送信した <b>{NUM_SITES} 件のサイト</b>を確認する}}すべてブロックする{NUM_SITES,plural, =1{1 件のサイトで通知をブロックしています}other{{NUM_SITES} 件のサイトで通知をブロックしています}}すべて完了していますページをプリロードして、閲覧と検索をすばやく行えるようにするCookie を使って設定を保存します（これらのページにアクセスしない場合も Cookie が保持されます）Chrome でアクセスが想定されるページをプリロードします。Cookie が許可されている場合は、プリロードの際に Cookie を使用します。また、サイトでアクセス元が特定されないように、Google 経由でページを暗号化して送信する場合があります。セーフ ブラウジング（危険なサイトからユーザーとデバイスを保護します）セキュリティ上のリスクがある場合に、アクセスした一部のページの URL が Google に送信されます新たな脅威の発見と、すべてのウェブユーザーの保護に役立てるため、アクセスした一部のページの URL、限定的なシステム情報、一部のページ コンテンツを Google に送信します。セーフ ブラウジング保護強化機能危険なウェブサイト、ダウンロード、拡張機能から予防的な方法で迅速に保護します。パスワードの侵害があった場合は警告します。Google に閲覧データを送信する必要があります。保護強化機能の詳細を表示危険なイベントの発生を予測して警告しますすべてのウェブユーザーの安全性を高めますデータ侵害によりパスワードが漏洩した場合に警告しますセーフ ブラウジングに URL を送信して安全性を確認します。また、新たな脅威を発見するため、ページ、ダウンロード、拡張機能のアクティビティ、システム情報の一部のサンプルも送信します。ログイン時には、このデータを一時的に Google アカウントにリンクして、すべての Google アプリを安全に利用できるように保護します。標準保護機能危険だと認識されているウェブサイト、ダウンロード、拡張機能から標準的な方法で保護します標準保護機能の詳細を表示すべてのユーザーのウェブ上のセキュリティ強化に協力する保護なし（推奨されません）危険なウェブサイト、ダウンロード、拡張機能から保護されていません。セーフ ブラウジングによる保護は、Gmail や Google 検索など他の Google サービスで利用可能な場合は、引き続き有効です。セーフ ブラウジングをオフにしますか？セーフ ブラウジングは、不正なソフトウェアのインストールや個人情報（パスワード、電話番号、クレジット カードなど）の入力といった危険な操作を行うよう誘導する悪意のあるユーザーから保護します。オフにする場合、なじみのないサイトや評判の悪いサイトを閲覧する際は十分に注意してください。オフにする閲覧トラフィックと一緒に「Do Not Track」リクエストを送信するDo Not Track「Do Not Track」を有効にすると、閲覧トラフィックにリクエストが含まれるようになります。ウェブサイトがリクエストに応答するかどうか、またリクエストがどのように解釈されるかによって、この影響は異なります。たとえば、このリクエストに対応して、ユーザーが過去にアクセスしたウェブサイトに関係なく広告を表示するウェブサイトもあります。トラッキングを拒否しても、多くのウェブサイトではユーザーの閲覧データが収集され利用されるのが現状です。たとえば、セキュリティの強化、ウェブサイトでのコンテンツ、サービス、広告、おすすめ情報の表示、レポート統計情報の作成などにこれらのデータが利用されます。<a target="_blank" href="$1">詳細</a>権限とコンテンツの設定サイトが使用、表示できる情報（位置情報、カメラ、ポップアップなど）を制御しますセキュリティセーフ ブラウジング（危険なサイトからの保護機能）などのセキュリティ設定Google の高度な保護機能プログラム個人の Google アカウントを標的型攻撃から保護します常に安全な接続を使用する移動先を HTTPS にアップグレードし、HTTPS をサポートしていないサイトは読み込む前に警告を表示します移動先を HTTPS にアップグレードし、HTTPS をサポートしていないサイトは読み込む前に警告を表示します。高度な保護機能が有効になっているため、この設定は変更できません。証明書の管理HTTPS / SSL の証明書と設定を管理しますデバイス証明書の管理デバイスの HTTPS / SSL 証明書を管理しますセキュア DNS を使用するウェブサイトに安全に接続する方法を決定します現在のサービス プロバイダを使用するセキュア DNS は使用できない場合があります自分で選択したプロバイダを使用するプロバイダ オプション次を使用このプロバイダの<a target="_blank" href="$1">プライバシー ポリシー</a>をご覧くださいこの設定は管理対象のブラウザでは無効です保護者による使用制限が有効なため、この設定は無効になっていますカスタム プロバイダを入力正しい形式の URL を入力してください有効なプロバイダであることを確認するか、しばらくしてからもう一度お試しくださいコンテンツの設定サイトの設定ウェブサイトに使用を許可する情報や、表示してもよいコンテンツを管理しますデータを削除データを消去しています…データを消去しました。閲覧履歴、Cookie、キャッシュなどを削除しますプライバシー、セキュリティ、データ収集に関連するその他の設定最近変更した権限はありません$1を許可$1、$2を許可しました$1、他 $2 件を許可しました$1を自動ブロック$1、$2を自動ブロックしました$1、他 $2 件を自動ブロックしました$1をブロック$1、$2をブロックしました$1、他 $2 件をブロックしました設定をリセットしますか？設定のリセット設定を元の既定値に戻す一部の設定がリセットされました設定の一部が別のプログラムによって変更されていたため、元のデフォルト設定に戻しました。すべての設定をリセット設定のリセットの詳細検索エンジンアドレスバーで使用する検索エンジンです。デフォルトの検索エンジンの詳細検索エンジンとサイト内検索を管理するアドレスバーにショートカットを入力して、特定のサイト内をすばやく検索したり、別の検索エンジンを使用したりできます検索エンジンの追加検索エンジンの編集検索エンジンを削除この検索エンジンを削除してもよろしいですか？既定と異なる検索エンジンを使用するには、アドレスバーに検索エンジンのショートカットを入力して、設定したキーボード ショートカットを押します。ここで既定の検索エンジンを変更することもできます。サイト内検索特定のサイトや Chrome のセクションを検索するには、アドレスバーにそれらのショートカットを入力して、設定したキーボード ショートカットを押します。サイトが追加されていません無効なショートカット保存したその他の検索エンジンがここに表示されます拡張機能によって Chrome に検索エンジンが追加される場合がありますサイトまたはページクエリ URLURL（%s=検索語句）有効にする無効にするキーボード ショートカットアドレスバーで、このキーボード ショートカットを検索エンジンやサイト内検索のショートカットとともに使用しますスペースキーまたは Tab キーTab追加の無効なサイト$1 に埋め込まれたページホストに埋め込まれたページすべてのサイトすべてのサイトに保存されている権限とデータを表示並べ替えよくアクセスするページ保存データ権限を削除サイトの詳細を表示分割—デフォルトの動作サイトにアクセスしたときにこの設定の動作を自動的に行いますサイトにアクセスしたときにこの設定の動作を自動的に行います。多くのサイトは、最新情報やチャット メッセージなどを知らせる目的で通知を送信します。動作のカスタマイズ次のサイトでは、デフォルトではなくカスタムの設定が使用されます次のサイトではカスタムの設定が使用されます多くのサイトは、コンテンツやサービスを料金なしで提供する目的で広告を表示します。ただし、一部のサイトについては、煩わしい広告や誤解を招く広告が表示されることがわかっています。アクセスしたすべてのサイトであらゆる広告の表示が許可されます煩わしい広告や誤解を招く広告が表示されることがわかっているサイトで広告がブロックされますあらゆる広告の表示を許可するサイト煩わしい広告や誤解を招く広告の表示を許可しないサイト多くのサイトは、ゲームや HUD のナビ表示などで AR 機能を使用するためにカメラ位置を追跡します。サイトがカメラ位置の追跡を要求できるようにするサイトにカメラ位置の追跡を許可しないカメラ位置の追跡を許可するサイトカメラ位置の追跡を許可しないサイト時間節約のために、関連する複数のファイルをまとめて自動的にダウンロードするサイトもありますサイトが複数のファイルの自動ダウンロードを要求できるようにするサイトに複数のファイルの自動ダウンロードを許可しない複数のファイルの自動ダウンロードを許可するサイト複数のファイルの自動ダウンロードを許可しないサイトユーザーがサイトから離れた後も、サイトは同期を継続して、写真のアップロードやチャット メッセージの送信などのタスクを完了することがあります最近閉じたサイトがデータの送受信を完了できるようにする閉じたサイトにデータの送受信の完了を許可しないページを閉じると、開始したタスクが完了できなくなりますデータの送受信の完了を許可するサイトデータの送受信の完了を許可しないサイトBluetooth Low Energy ビーコン、ヘルス トラッカー、フィットネス トラッカー、スマート電球の設定や同期などの機能に対応するために、多くのサイトは Bluetooth デバイスに接続しますサイトが Bluetooth デバイスへの接続を要求できるようにするサイトに Bluetooth デバイスへの接続を許可しない多くのサイトは、ビデオチャットなどの通信機能にビデオカメラを使用します。サイトがカメラの使用を要求できるようにするサイトにカメラの使用を許可しないカメラを必要とする機能は動作しなくなりますカメラの使用を許可するサイトカメラの使用を許可しないサイト多くのサイトは、コピーしたテキストの書式を保持するなどのためにクリップボードを読み取ります。サイトがクリップボード内のテキストや画像へのアクセスを要求できるようにするサイトにクリップボード内のテキストや画像へのアクセスを許可しないクリップボードの読み取りを許可するサイトクリップボードの読み取りを許可しないサイト多くのサイトはデバイスのアクティブ状態を検出して、チャットアプリのステータスを設定していますサイトがデバイスのアクティブ状態の検出を要求できるようにするサイトによるデバイスのアクティブ状態の検出を許可しないこのデバイスのアクティブ状態の検出を許可するこのデバイスのアクティブ状態の検出を許可しないID サービスで使用しているアカウントでウェブサイトにログインできますサイトで ID サービスからのログイン メッセージを表示できるようにするID サービスからのログイン メッセージをブロックするサードパーティのログイン メッセージの表示を許可するサイトサードパーティのログイン メッセージの表示を許可しないサイト多くのサイトは、作業内容を自動的に保存するためにデバイスのファイルやフォルダにアクセスします。サイトがデバイスのファイルやフォルダの編集を要求できるようにするサイトにデバイス上のファイルやフォルダの編集を許可しないデバイスのファイルやフォルダの編集を許可しないサイト多くのサイトはデバイスのフォントを使用して、オンラインのデザインツールや画像編集ツールで高品質のコンテンツを作成できるようにしていますサイトがデバイスにインストールされたフォントの使用を要求できるようにするデバイスにインストールされたフォントの使用をサイトに許可しないデバイスにインストールされたフォントの使用を許可するデバイスにインストールされたフォントの使用を許可しない一般的ではないキーボードや、ゲーム コントローラなどのデバイスを使用する機能に対応するために、多くのサイトは HID デバイスに接続しますサイトが HID デバイスへの接続を要求できるようにするサイトに HID デバイスへの接続を許可しない多くのサイトは、オンライン ショップやニュース記事の写真や挿絵として使用するために画像を表示します。サイトが画像を表示できるようにするサイトに画像の表示を許可しない画像を必要とする機能は動作しなくなります画像の表示を許可するサイト画像の表示を許可しないサイトセキュリティで保護されたサイトに、セキュリティで保護されていない画像やウェブフレームなどのコンテンツが埋め込まれている場合があります安全でないコンテンツの表示を許可するサイト安全でないコンテンツの表示を許可しないサイト多くのサイトは、インタラクティブなビデオゲームやウェブフォームなどを表示するために JavaScript を使用しますサイトが JavaScript を使用できるようにするサイトに JavaScript の使用を許可しないJavaScript の使用を許可するサイトJavaScript の使用を許可しないサイト多くのサイトは、ローカル ニュースや近くのお店の情報などを取得するためにユーザーの位置情報を使用します。位置情報へのアクセス権限がサイトからリクエストされる場合がありますサイトに位置情報へのアクセスを許可しない位置情報を必要とする機能は動作しなくなります位置情報の取得を許可するサイト位置情報の取得を許可しないサイト多くのサイトは、ビデオチャットなどの通信機能にマイクを使用します。サイトがマイクの使用を要求できるようにするサイトにマイクの使用を許可しないマイクを必要とする機能は動作しなくなりますマイクの使用を許可するサイトマイクの使用を許可しないサイト多くのサイトは、音楽の制作や編集をするために MIDI デバイスに接続します。サイトが MIDI デバイスへの接続を要求できるようにするサイトに MIDI デバイスへの接続を許可しないMIDI デバイスへの接続を許可するサイトMIDI デバイスへの接続を許可しないサイト多くのサイトは、バーチャル リアリティやフィットネス トラッキングなどの機能にデバイスのモーション センサーを使用しますサイトがモーション センサーを使用できるようにするサイトにモーション センサーの使用を許可しないモーション センサーを必要とする機能は動作しなくなりますモーション センサーの使用を許可するサイトモーション センサーの使用を許可しないサイトサイトは最新情報やチャット メッセージなどを知らせる目的で通知を送信します通知を送信するかどうかの確認をサイトに許可する目立たない方法で通知する通知を送信するかどうかをサイトが確認できないようにしますサイトに通知の送信を許可しない通知を必要とする機能は動作しなくなります通知の送信を許可するサイト通知の送信を許可しないサイト多くのサイトは、お支払い手続きを簡易化するなど、ショッピングをより快適にするために支払いハンドラをインストールします。サイトが支払いハンドラをインストールできるようにするサイトに支払いハンドラのインストールを許可しない支払いハンドラのインストールを許可するサイト支払いハンドラのインストールを許可しないサイト一部のサイトはドキュメント、契約、フォームなどの PDF を一般に提供していますPDF をダウンロードするポップアップを送信して広告を表示したり、リダイレクトを使ってユーザーがアクセスするつもりのないウェブサイトを表示したりするサイトもありますサイトがポップアップを送信したりリダイレクトを使用したりできるようにするサイトにポップアップの送信やリダイレクトの使用を許可しないポップアップの送信やリダイレクトの使用を許可するサイトポップアップの送信やリダイレクトの使用を許可しないサイト著作権で保護されたコンテンツを再生するには、サイトでコンテンツ保護サービスの使用が必要となる場合があります保護されたコンテンツの再生をサイトに許可するサイトに保護されたコンテンツの再生を許可しない著作権が設定されたメディアは再生できませんサイトで識別子を使用したデバイスの認識も必要となる場合があります識別子の使用と保護されたコンテンツの再生をサイトに許可する識別子の使用と保護されたコンテンツの再生をサイトに許可しないメディアの品質が低下する可能性があります識別子の使用と保護されたコンテンツの再生を許可するサイト識別子の使用と保護されたコンテンツの再生を許可しないサイトChrome 自動字幕起こしを使用できない可能性があります特定の種類のリンクをクリックしたときに、メール クライアントで新しいメールを作成したり、オンライン カレンダーに新しい予定を追加したりといったタスクをサイトで処理できるようにしますサイトがプロトコルの処理を要求できるようにするサイトにプロトコルの処理を許可しないプロトコルの処理を許可しないサイト多くのサイトは、ネットワークを設定するなど、データ転送機能を使用するためにシリアルポートに接続しますサイトがシリアルポートへの接続を要求できるようにするサイトにシリアルポートへの接続を許可しない音楽や動画などのメディアの音声を再生するサイトもあります。サイトが音声を再生できるようにするサイトに音声の再生を許可しない音を使用する機能は動作しなくなります音声の再生を許可するサイト音声の再生を許可しないサイト多くのサイトは、ドキュメントを印刷したりストレージ デバイスに保存するために USB デバイスに接続します。サイトが USB デバイスへの接続を要求できるようにするサイトに USB デバイスへの接続を許可しない多くのサイトは、VR セッションに参加するためにバーチャル リアリティ デバイスとデータを使用します。サイトがバーチャル リアリティ デバイスとデータの使用を要求できるようにするサイトにバーチャル リアリティ デバイスとデータの使用を許可しないバーチャル リアリティ デバイスとデータの使用を許可するサイトバーチャル リアリティのデバイスやデータの使用を許可しないサイト特定のウェブサイトをどの程度拡大または縮小して表示するかを設定できますサイトから周囲の 3D マップの作成またはカメラ位置の追跡を求められたときに確認するサイトに周囲の 3D マップの作成またはカメラ位置の追跡を許可しないアクセスしたサイトに他のサイトのコンテンツ（画像、広告、テキストなど）が埋め込まれている場合があります。こうした他のサイトによって設定された Cookie は「サードパーティ Cookie」と呼ばれます。サードパーティ Cookie の使用が許可されていますサードパーティ Cookieサードパーティの Cookie が許可されていますシークレット モードでサードパーティの Cookie がブロックされていますサードパーティの Cookie がブロックされていますサードパーティの Cookie を許可サードパーティ Cookie の許可に関する詳細シークレット モードでサードパーティ Cookie をブロックするシークレット モードでのサードパーティ Cookie のブロックに関する詳細シークレット モードのとき、サイトは Cookie を使用して他のサイトでの閲覧アクティビティを確認することができません。閲覧アクティビティは、広告のパーソナライズなどには使用されません。一部のサイトで機能を使用できなくなる可能性があります。サードパーティの Cookie をブロックするサードパーティ Cookie のブロックの詳細サイトは、Cookie を使用して別のサイトでのあなたの閲覧アクティビティを確認できず、広告のパーソナライズなどが行えなくなります。一部のサイトで機能を使用できなくなる可能性があります全般設定Cookie をすべて受け入れるすべての Cookie の許可について詳細を表示するシークレット モードでのサードパーティの Cookie のブロックについて詳細を表示するシークレット モードのとき、サイトは Cookie を使用して別のサイトでのあなたの閲覧アクティビティを確認できず、広告のパーソナライズなどを行えなくなります。一部のサイトで機能を使用できなくなる可能性があります。シークレット モードのとき、サイトは Cookie を使用して関連サイトを含む他のサイトでの閲覧アクティビティを確認することができません。閲覧アクティビティは、広告のパーソナライズなどには使用されません。一部のサイトで機能を使用できなくなる可能性があります。サードパーティの Cookie のブロックについて詳細を表示するサイトは、Cookie を使用して別のサイトでのあなたの閲覧アクティビティを確認できず、広告のパーソナライズなどが行えなくなります。一部のサイトで機能を使用できなくなる可能性があります。すべての Cookie をブロックする（推奨されません）すべての Cookie のブロックについて詳細を表示するサイトは、Cookie を使用してブラウジングの利便性を改善できません（ログイン状態の維持、ショッピング カートの中身の保存などが行えません）サイトは、Cookie を使用して別のサイトでのあなたの閲覧アクティビティを確認できません（広告のパーソナライズなどが行えません）多くのサイトで機能を使用できなくなる可能性があります関連サイトにグループ内のアクティビティの確認を許可する企業は、Cookie を使用してグループ内のアクティビティを共有できるサイトのグループを定義できます。シークレット モードの場合、この設定はオフになっています。すべてのウィンドウを閉じるときに Cookie とサイトデータを削除するすべてのサイトデータと権限を表示常に Cookie を使用できるサイトウィンドウを閉じるときに常に Cookie を削除Cookie を使用できないサイト以下のアプリでもプロトコル リンクを処理できます。他のアプリでは許可が必要となります。以下のアプリはプロトコル リンクを扱いません。サイトから HID デバイスへのアクセス許可を求められたときに確認するHID デバイスへのアクセスをどのサイトにも許可しない保護されているサイトでは、保護されていないコンテンツはデフォルトでブロックされます。PDF ファイルを Chrome で自動的に開く代わりにダウンロードする最近のアクティビティ拡大縮小値の指定を削除送信する前に確認する{COUNT,plural, =0{Cookie が許可されています}=1{Cookie が許可されています（1 件の例外）}other{Cookie が許可されています（{COUNT} 件の例外）}}{COUNT,plural, =0{Cookie がブロックされています}=1{Cookie がブロックされています（1 件の例外）}other{Cookie がブロックされています（{COUNT} 件の例外）}}サイトに Cookie データの保存と読み取りを許可するサイトからのプロトコルのデフォルト ハンドラとしての登録要求を許可する（推奨）すべてのサイトに対してプロトコルの処理を許可しないサイトは通常、ウィンドウをインテリジェントに開いて配置できるように（ドキュメントや全画面コンテンツを並べて表示するなど）、ディスプレイに関する情報をリクエストします全ディスプレイのウィンドウ管理をリクエストすることをサイトに許可する全ディスプレイのウィンドウ管理をサイトに許可しない全ディスプレイのウィンドウ管理が許可されています全ディスプレイのウィンドウ管理が許可されていません許可する終了時に消去確認（デフォルト）許可（デフォルト）自動（デフォルト）ブロック（デフォルト）ミュート（デフォルト）確認するミュート使用状況使用状況データはありませんその他の権限コンテンツその他のコンテンツの設定内部で許可リストに登録済みこのサイトでは煩わしい広告や誤解を招く広告が表示されます煩わしい広告や誤解を招く広告が表示されるサイトの場合にブロックしますセキュリティ保護のために一時的にブロックされていますプライバシー保護のためにブロックされています権限をリセットサイト上のサードパーティ Cookie も対象データをすべて消去しますか？表示されたサイトのデータを削除しますか？これらのサイトは、$1 により定義されたグループに属しています。グループ内のサイトは、グループでのアクティビティを確認できます。新しいタブで関連サイトの詳細を確認しますサイトの合計ストレージ使用状況: $1表示されたサイトの合計ストレージ使用状況: $1データをすべて消去表示データを消去$1 のサイトの詳細を表示{MEMBERS,plural, =1{{FPS_OWNER} のグループ内の 1 件のサイト}other{{FPS_OWNER} のグループ内の {MEMBERS} 件のサイト}}同じグループ内のサイトを表示するデータと権限を消去するサイトにより保存された $1 のデータが削除されます表示中のサイトにより保存された $1 のデータが削除されますサイトにより保存された $1 のデータとインストールされたアプリが削除されます表示されたサイトとインストールされたアプリによって保存された $1 のデータが削除されますタブで表示中のサイトも含め、すべてのサイトからログアウトします開いているタブを含め、表示されたサイトからログアウトします$1 のサイト権限をリセットしますか？サイトデータを削除しますか？$1 で保存されたすべてのデータが削除されます。$1 により保存されたすべてのデータと Cookie が削除されます。開いているすべてのタブで、このサイトからログアウトしますオフライン データも削除されます広告のパーソナライズに影響するデータは削除されます$1 のサイトデータと権限を削除しますか？$1 のサイトデータと権限、およびそのサイトのインストール済みのアプリを削除しますか？$2 で分割された $1 のサイトデータを削除しますか？$1 とその下にあるすべてのサイトのサイトデータと権限を削除しますか？$1 とその下にあるすべてのサイトのサイトデータと権限、およびそのサイトのインストール済みのアプリを削除しますか？$1 とその下にあるすべてのサイトのサイトデータと権限、およびインストール済みのアプリを削除しますか？タブで表示中のものも含め、このサイトからログアウトしますタブで表示中のものも含め、これらのサイトからログアウトします選択した権限がリセットされます$1 を削除リセットデフォルトとして設定現在のシークレット モード セッションのみこのカスタム設定はシークレット ウィンドウをすべて閉じると削除されます拡大縮小値を設定したサイトはありませんサイトで音声が再生される可能性がある場合、Chrome で自動的に許可またはブロックする（推奨）アクセスしたサイトがここに表示ckeditor4/pull/2062): Added the emoji dropdown that allows the user to choose the emoji from the toolbar and search for them using keywords.
